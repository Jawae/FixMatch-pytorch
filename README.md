...
- With 4 GPUs, **running statistics of BN** is not gahtered in distributed training for fast update. But, larger number of GPUs with same batch size might affect overall accuracies. Then, you can 1) replace BN to syncBN (see [here](https://pytorch.org/docs/stable/generated/torch.nn.SyncBatchNorm.html)) or 2) use `torch.distributed.all_gather` before [this line](https://github.com/LeeDoYup/FixMatch-pytorch/blob/a338e335b2036cb8b1ba067403faac0dfb930d1e/models/fixmatch/fixmatch.py#L80).
